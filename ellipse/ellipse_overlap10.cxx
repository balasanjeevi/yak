                 ...
                 int nrhs, const mxArray *prhs[])
{
	...
	if(dims1[0]<9 || dims2[0]<9 || dims1[0]!=dims2[0] || dims3[0]!=1){ 
		...
		if (dims1[0]!=dims2[0]) 	printf(" dims1[0]!=dims2[0] %d!=%d ",dims1[0],dims2[0]);
		if (dims3[0]!=1) 			printf(" dims3[0]!=1 %d ",dims3[0]);
		printf("dims != 9\n"); return;
	}
	...
	//plhs [5] = mxCreateNumericArray(3, odim_bounding_rect, mxDOUBLE_CLASS, mxREAL);
	...
	//double       *parea_out 	= (double *)mxGetData(plhs[1]);	// intersection area
	//double       *pdr_out 		= (double *)mxGetData(plhs[2]);	// resolution / sampling
	//double       *pbna_out 		= (double *)mxGetData(plhs[3]);	// intersection area in terms of sample points
	//double       *pbua_out 		= (double *)mxGetData(plhs[4]);	// total area in terms of sample points
	//double       *prect_bounding= (double *)mxGetData(plhs[5]);	// bounding rectangle
	...
	for(int j=0,f1=0;j<dims1[1];j++,f1+=dims1[0]) {
		...
		for (int i=0,f2=0; i<dims2[1]; i++,f2+=dims1[0]) {
			...
			if(dist<max_dist) {
				...
				bua=0;bna=0;int t1=0,t2=0; f_area1=0; f_area2=0;
				//compute the area
